...
def finviz_screen() -> list[str]:
    ...
    # الطريقة الأولى: فينفيز يخبئ الرموز في تعليق TS...TE في نهاية الصفحة
    # بصيغة  TICKER|السعر|الفوليوم  — أنظف من تحليل الجدول وأقل عرضة للكسر
    block = re.search(r"<!--\s*TS\s*(.*?)\s*TE\s*-->", html, re.S)
    if block:
    ...
    # الطريقة الثانية (احتياطية): تحليل جدول النتائج
    try:
        for tb in pd.read_html(StringIO(html)):   # StringIO مطلوب في pandas الحديثة
            if "Ticker" in tb.columns:
                return [str(t).strip().upper() for t in tb["Ticker"].dropna()]
    except Exception as e:
        print(f"تحليل الجدول فشل: {e}", file=sys.stderr)

    print("لم أجد نتائج في صفحة فينفيز.", file=sys.stderr)
    return []
